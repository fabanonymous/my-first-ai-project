...
public class Rag2Configuration {
    ...
    public ChatClient chatClient(MistralAiChatModel chatModel, @Qualifier("VectorStoreForRag2") VectorStore vectorStore) {
        return ChatClient.builder(chatModel)
                .defaultAdvisors(new QuestionAnswerAdvisor(vectorStore))
                .build();
    }

    @Bean(name="VectorStoreForRag2")
    public PgVectorStore vectorStore(JdbcTemplate jdbcTemplate, MistralAiEmbeddingModel embeddingModel, PgVectorStoreProperties properties, ObjectProvider<ObservationRegistry> observationRegistry, ObjectProvider<VectorStoreObservationConvention> customObservationConvention, BatchingStrategy batchingStrategy) {
